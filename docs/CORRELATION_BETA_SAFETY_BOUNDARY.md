# Correlation and Beta Safety Boundary

Prompt 31 keeps relationship metrics descriptive/research-only.

## Safety Rules

- Correlation is not a signal.
- Beta is not a signal.
- A relationship metric is not a recommendation.
- No thresholds or regime labels are implemented.
- No buy/sell/hold/watch/avoid output is implemented.
- No DecisionObject generation is implemented.
- No execution APIs are implemented.
- No trading interpretation is implemented.
- No broker integration is implemented.
- No event publishing to decision or execution systems is implemented.

## API Boundary

The `/relationship-analytics/health` and `/relationship-analytics/contracts` endpoints expose metadata only. They do not accept user-supplied paired vectors and do not compute analytics for API callers.

## Forbidden Scope

Prompt 31 does not implement indicators, feature computation, factors, backtesting, regimes, signals, recommendations, decisions, real ingestion, external calls, scraping, credentials, provider SDKs, or execution APIs.

Explicit audit phrases:

- no indicators.
- no backtesting.
- no regimes.
- no signals.
- no recommendations.
- no DecisionObject generation.
- no execution APIs.

Development verification runs on Mac mini M2 / macOS / Apple Silicon. The target desktop product remains Windows-native Stark Terminal.

