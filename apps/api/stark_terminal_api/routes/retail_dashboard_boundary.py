...
from stark_terminal_core.retail_dashboard_boundary.health import check_retail_dashboard_boundary_health
...
@router.get("/retail-dashboard-boundary/health")
def retail_dashboard_boundary_health() -> dict[str, Any]:
    status = check_retail_dashboard_boundary_health(get_settings())
    return {
        "service": "stark-terminal-retail-dashboard-boundary",
        **status.model_dump(),
    }


@router.get("/retail-dashboard-boundary/contracts")
def retail_dashboard_boundary_contracts() -> dict[str, Any]:
    ...
    return {
        "service": "stark-terminal-retail-dashboard-boundary",
        "schema_version": settings.retail_dashboard_boundary_schema_version,
...
@router.get("/retail-dashboard-boundary/invariants")
def retail_dashboard_boundary_invariants() -> dict[str, Any]:
