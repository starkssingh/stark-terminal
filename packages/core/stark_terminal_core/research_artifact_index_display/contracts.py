...
class ResearchArtifactIndexDisplayContract(BaseModel):
    contract_id: str
    service: str = "stark-terminal-research-artifact-index-display"
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research artifact index display contract text fields")
    ...
    def contract_must_fail_closed(self) -> ResearchArtifactIndexDisplayContract:
        if self.stage != "display_contract_skeleton":
            raise ValueError("Research Artifact Index Display contract stage must be display_contract_skeleton")
        if not self.read_only:
            raise ValueError("Research Artifact Index Display contract must be read-only")
        if not self.unavailable_by_default:
            raise ValueError("Research Artifact Index Display contract must be unavailable by default")
        dangerous_flags = {
        ...
        if enabled:
            raise ValueError(f"Research Artifact Index Display contract cannot enable: {', '.join(enabled)}")
        return self


def default_research_artifact_index_display_contract() -> ResearchArtifactIndexDisplayContract:
    return ResearchArtifactIndexDisplayContract(
        contract_id="research-artifact-index-display-contract-v1",
    )
