...
class ResearchArtifactIndexDisplayUnavailableResponse(BaseModel):
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research artifact index display unavailable response text fields")
    ...
    def unavailable_response_must_fail_closed(self) -> ResearchArtifactIndexDisplayUnavailableResponse:
        if not self.unavailable:
            raise ValueError("Research Artifact Index Display unavailable response must remain unavailable")
        if self.allowed_stage != "display_contract_skeleton":
            raise ValueError("Research Artifact Index Display unavailable response stage must be display_contract_skeleton")
        dangerous_flags = {
        ...
        if enabled:
            raise ValueError(f"Research Artifact Index Display unavailable response cannot enable: {', '.join(enabled)}")
        return self


def unavailable_display_response_template() -> ResearchArtifactIndexDisplayUnavailableResponse:
    return ResearchArtifactIndexDisplayUnavailableResponse(
        reason=(
            "Research Artifact Index Display is a backend display contract skeleton; active UI, "
            "frontend/desktop components, indexing, search, ranking, retrieval, embeddings, vector store, "
            "ingestion, storage, file upload/download/preview, paper parsing, strategy generation, "
