...
class ResearchMetadataGraphReadinessStatus(BaseModel):
    status_id: str = "research-metadata-graph-readiness-v1"
    ready_for_planning: bool = True
    ready_for_api_contract_skeleton: bool = False
    ready_for_graph_database: bool = False
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research metadata graph readiness text")
    ...
    def readiness_must_remain_planning_only(self) -> ResearchMetadataGraphReadinessStatus:
        if not self.ready_for_planning:
            raise ValueError("research metadata graph must remain ready for planning")
        dangerous_readiness = {
            "graph database": self.ready_for_graph_database,
        ...
        if enabled:
            raise ValueError("research metadata graph readiness cannot allow: " + ", ".join(enabled))
        if self.ready_for_api_contract_skeleton:
            raise ValueError("Research Metadata Graph API contract skeleton readiness is reserved for Prompt 85")
        if self.next_allowed_phase != "api_contract_skeleton":
            raise ValueError("Research Metadata Graph next phase must be api_contract_skeleton")
        return self


def research_metadata_graph_readiness(settings: object | None = None) -> ResearchMetadataGraphReadinessStatus:
    schema_version = getattr(settings, "research_metadata_graph_schema_version", "v1")
    return ResearchMetadataGraphReadinessStatus(schema_version=schema_version)
