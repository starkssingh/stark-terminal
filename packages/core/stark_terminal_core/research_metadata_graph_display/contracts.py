...
from stark_terminal_core.research_metadata_graph_display.init import SERVICE_NAME


class ResearchMetadataGraphDisplayContract(BaseModel):
    contract_id: str
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research metadata graph display contract text")
    ...
    def contract_must_fail_closed(self) -> ResearchMetadataGraphDisplayContract:
        if self.service != SERVICE_NAME:
            raise ValueError("Research Metadata Graph Display service name is fixed")
        if self.stage != "display_contract_skeleton":
            raise ValueError("Research Metadata Graph Display stage must be display_contract_skeleton")
        if not self.read_only:
            raise ValueError("Research Metadata Graph Display contract must be read-only")
        if not self.unavailable_by_default:
            raise ValueError("Research Metadata Graph Display contract must be unavailable-by-default")
        if not self.display_contract_skeleton_only:
            raise ValueError("Research Metadata Graph Display contract must remain skeleton-only")
        dangerous_flags = {
        ...
        if enabled:
            raise ValueError("Research Metadata Graph Display contract cannot enable: " + ", ".join(enabled))
        return self


def default_research_metadata_graph_display_contract(
    ...
    return ResearchMetadataGraphDisplayContract(
        contract_id="research-metadata-graph-display-contract-v1",
