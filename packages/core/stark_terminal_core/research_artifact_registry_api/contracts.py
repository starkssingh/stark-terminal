...
class ResearchArtifactRegistryApiContract(BaseModel):
    contract_id: str
    service: str = "stark-terminal-research-artifact-registry-api"
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research artifact registry API contract text fields")
    ...
    def contract_must_fail_closed(self) -> ResearchArtifactRegistryApiContract:
        if self.stage != "api_contract_skeleton":
            raise ValueError("Research Artifact Registry API contract stage must be api_contract_skeleton")
        if not self.read_only:
            raise ValueError("Research Artifact Registry API contract must be read-only")
        if not self.unavailable_by_default:
            raise ValueError("Research Artifact Registry API contract must be unavailable by default")
        dangerous_flags = {
        ...
        if enabled:
            raise ValueError(f"Research Artifact Registry API contract cannot enable: {', '.join(enabled)}")
        return self


def default_research_artifact_registry_api_contract() -> ResearchArtifactRegistryApiContract:
    return ResearchArtifactRegistryApiContract(
        contract_id="research-artifact-registry-api-contract-v1",
    )
