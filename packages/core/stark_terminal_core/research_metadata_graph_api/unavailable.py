...
class ResearchMetadataGraphApiUnavailableResponse(BaseModel):
    ...
    def text_fields_must_be_non_empty(cls, value: str) -> str:
        return non_empty_text(value, "research metadata graph API unavailable response text")
    ...
    def unavailable_response_must_fail_closed(self) -> ResearchMetadataGraphApiUnavailableResponse:
        if not self.unavailable:
            raise ValueError("Research Metadata Graph API unavailable response must remain unavailable")
        if self.allowed_stage != "api_contract_skeleton":
            raise ValueError("Research Metadata Graph API unavailable response stage must be api_contract_skeleton")
        dangerous_flags = {
        ...
        if enabled:
            raise ValueError("Research Metadata Graph API unavailable response cannot enable: " + ", ".join(enabled))
        return self


def unavailable_graph_api_response_template() -> ResearchMetadataGraphApiUnavailableResponse:
    return ResearchMetadataGraphApiUnavailableResponse(
        reason=(
            "Research Metadata Graph API is a read-only contract skeleton; graph database, persistent writes, "
            "traversal, query, search, ranking, retrieval, embeddings, vector store, ingestion, upload/download/"
            "preview, paper parsing, strategy generation, backtesting, recommendations, and execution are unavailable."
        ),
    )
