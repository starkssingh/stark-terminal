...
def test_retail_dashboard_boundary_health_endpoint_is_safe() -> None:
    response = client.get("/retail-dashboard-boundary/health")
    assert response.status_code == 200
    body = response.json()

    assert body["service"] == "stark-terminal-retail-dashboard-boundary"
...
def test_retail_dashboard_boundary_contracts_endpoint_returns_policy_metadata() -> None:
    response = client.get("/retail-dashboard-boundary/contracts")
    assert response.status_code == 200
    body = response.json()
    ...
    assert "retail-dashboard-boundary" in body["endpoint_families"]
    assert "retail_dashboard_boundary" in body["module_families"]
...
def test_retail_dashboard_boundary_invariants_endpoint_returns_safe_result() -> None:
    response = client.get("/retail-dashboard-boundary/invariants")
    assert response.status_code == 200
    body = response.json()
...
def test_retail_dashboard_boundary_has_no_post_endpoints() -> None:
    for route in app.routes:
        path = getattr(route, "path", "")
        methods = getattr(route, "methods", set())
        if path.startswith("/retail-dashboard-boundary"):
