...
def test_research_knowledge_map_get_endpoints_work() -> None:
    for endpoint in KNOWLEDGE_MAP_ENDPOINTS:
        response = client.get(endpoint)
        assert response.status_code == 200, endpoint
        body = response.json()
        assert body["service"] == "stark-terminal-research-knowledge-map"
...
def test_research_knowledge_map_routes_are_get_only() -> None:
    routes = [
        route
...
def test_research_knowledge_map_routes_have_no_unsafe_paths() -> None:
    forbidden_segments = [
        "broker",
        "order",
        "execution",
