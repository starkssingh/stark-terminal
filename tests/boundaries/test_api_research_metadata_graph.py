...
def test_research_metadata_graph_get_endpoints_work() -> None:
    for endpoint in GRAPH_ENDPOINTS:
        response = client.get(endpoint)
        assert response.status_code == 200, endpoint
        body = response.json()
        assert body["service"] == "stark-terminal-research-metadata-graph"
...
def test_research_metadata_graph_routes_are_get_only() -> None:
    routes = [
        route
...
def test_research_metadata_graph_routes_have_no_unsafe_paths() -> None:
    forbidden_segments = [
        "broker",
        "order",
        "execution",
