...
def test_retail_decision_console_static_state_view_model_endpoint_is_safe() -> None:
    response = client.get("/retail-decision-console/static-state-view-model")

    assert response.status_code == 200
    body = response.json()
    assert body["service"] == "stark-terminal-retail-decision-console"
    assert body["static_state_wiring_only"] is True
    ...
    assert body["view_model"]["stage"] == "static_state_wired_shell"
    assert body["view_model"]["demo_only"] is True
    assert body["view_model"]["unavailable"] is True
    assert body["view_model"]["sections"]
    _assert_no_secrets_or_enabled_dangerous_flags(body)


def test_retail_decision_console_static_state_wiring_route_family_is_get_only() -> None:
    routes = [
        route
    ...
    assert "/retail-decision-console/static-state-view-model" in discovered
    for route in routes:
        assert getattr(route, "methods", set()) <= {"GET"}


def test_retail_decision_console_static_state_wiring_adds_no_unsafe_routes() -> None:
    forbidden_segments = [
        "broker",
        "order",
        "execution",
