...
def test_research_knowledge_map_api_get_endpoints_work() -> None:
    for endpoint in KNOWLEDGE_MAP_API_ENDPOINTS:
        response = client.get(endpoint)
        assert response.status_code == 200, endpoint
        body = response.json()
        assert body["service"] == "stark-terminal-research-knowledge-map-api"
...
def test_research_knowledge_map_api_routes_are_get_only() -> None:
    routes = [
        route
        for route in _iter_routes(app.routes)
        if getattr(route, "path", "").startswith("/research-knowledge-map-api")
    ]
    discovered = {getattr(route, "path", "") for route in routes}
    assert set(KNOWLEDGE_MAP_API_ENDPOINTS) <= discovered
    for route in routes:
        assert getattr(route, "methods", set()) <= {"GET"}


def test_research_knowledge_map_api_routes_have_no_unsafe_paths() -> None:
    forbidden_segments = [
        "broker",
        "order",
        "execution",
    ...
    for route in _iter_routes(app.routes):
        ...
        if not path.startswith("/research-knowledge-map-api"):
            continue
