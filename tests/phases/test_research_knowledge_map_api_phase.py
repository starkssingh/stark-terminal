...
def test_prompt_90_research_knowledge_map_api_docs_and_status() -> None:
    ...
    assert "prompt 90" in combined_lower
    assert "research knowledge map api contract skeleton" in combined_lower
    ...
    assert "api safety helpers" in combined_lower
    assert "get-only read-only api contract metadata endpoints" in combined_lower
...
def test_prompt_90_added_no_micro_audit_sprawl() -> None:
    forbidden_docs = [
