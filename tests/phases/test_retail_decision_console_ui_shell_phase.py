...
def test_retail_decision_console_ui_shell_phase_is_recorded() -> None:
    ...
    assert "Prompt 96" in phase_doc
    assert "UI Shell Skeleton" in phase_doc
    assert "desktop shell scope" in phase_doc
    assert "no live data" in phase_doc
    assert "Prompt 96 follows the grouped docs/tests policy" in consolidation
    assert "Current Prompt: 96" in north_star
    assert "Retail Decision Console Status: UI shell skeleton only" in north_star
    assert "Prompt 96 - Retail Decision Console UI Shell Skeleton" in prompt_log
    assert "Prompt 97 - Retail Decision Console Demo Data Contract and Static State Model" in next_phase


def test_prompt_96_added_no_micro_audit_doc_sprawl() -> None:
    top_level_docs = [
