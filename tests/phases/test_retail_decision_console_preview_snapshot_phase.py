...
def test_retail_decision_console_phase_doc_records_prompt_102_preview_snapshot_export() -> None:
    phase_doc = _read("docs/phases/retail_decision_console.md")
    consolidation = _read("docs/testing/CONSOLIDATION_MAP.md")

    assert "Prompt 102 Preview Snapshot Export" in phase_doc
    assert "Retail Decision Console Preview Snapshot Export" in phase_doc
    assert "preview_snapshot_export" in phase_doc
    assert "--print-snapshot" in phase_doc
    assert "--export-snapshot" in phase_doc
    assert "Prompt 102 follows the grouped docs/tests policy" in consolidation


def test_prompt_102_status_docs_recommend_prompt_103() -> None:
    north_star = _read("docs/NORTH_STAR.md")
    prompt_log = _read("docs/PROMPT_LOG.md")
    next_phase = _read("docs/NEXT_PHASE_PLAN.md")

    assert "Current Prompt: 102" in north_star
    assert "Current Milestone: Retail Decision Console Productization - Preview Snapshot Export" in north_star
    assert "Prompt 102 - Retail Decision Console Preview Snapshot Export" in prompt_log
    assert "Prompt 103 - Retail Decision Console Local QA Bundle" in next_phase
    assert "Prompt 103 - Retail Decision Console Local QA Bundle" in prompt_log


def test_prompt_102_adds_no_micro_audit_doc_or_test_sprawl() -> None:
