...
def test_retail_decision_console_local_preview_phase_is_recorded() -> None:
    ...
    assert (ROOT / "docs/runbooks/retail_decision_console_local_preview.md").exists()
    assert (ROOT / "docs/runbooks/retail_decision_console_manual_smoke_test.md").exists()
    assert "Prompt 99" in phase_doc
    assert "Local Preview Runbook and Manual Smoke Test" in phase_doc
    assert "Prompt 99 follows the grouped docs/tests policy" in consolidation
    assert "Current Prompt: 99" in north_star
    assert "Retail Decision Console Status: static/demo shell previewable locally" in north_star
    assert "Prompt 99 - Retail Decision Console Local Preview Runbook and Manual Smoke Test" in prompt_log
    assert "Prompt 100 - Retail Decision Console Visual Polish and Section Layout Pass" in next_phase


def test_prompt_99_added_no_micro_audit_doc_sprawl() -> None:
    top_level_docs = [
...
def test_retail_decision_console_local_preview_preserves_execution_boundary_docs() -> None:
    no_execution = _read("docs/audits/no_execution.md")
    target_docs = [
    ...
    assert "execution apis remain forbidden" in no_execution.lower()
