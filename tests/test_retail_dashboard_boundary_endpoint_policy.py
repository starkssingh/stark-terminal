...
def _policy(**overrides: object) -> RetailDashboardEndpointBoundaryPolicy:
    data = {
        "policy_id": "retail-dashboard-test-boundary-policy",
        "endpoint_family": "retail-dashboard-test",
        "allowed_methods": ["GET"],
        "forbidden_methods": ["POST", "PUT", "PATCH", "DELETE"],
        "forbidden_outputs": list(DEFAULT_RETAIL_DASHBOARD_FORBIDDEN_ENDPOINT_OUTPUTS),
    }
    data.update(overrides)
    return RetailDashboardEndpointBoundaryPolicy(**data)


def test_default_endpoint_boundary_policies_validate() -> None:
    policies = default_retail_dashboard_endpoint_boundary_policies()
    ...
    assert evaluate_retail_dashboard_endpoint_boundary_policies(policies) == []
...
@pytest.mark.parametrize(
    "field_name",
    [
        "accepts_market_data_for_dashboard_decision",
...
def test_endpoint_policy_rejects_unknown_forbidden_output() -> None:
    with pytest.raises(ValidationError):
        _policy(forbidden_outputs=[RetailDashboardForbiddenBehaviorKind.UNKNOWN])
